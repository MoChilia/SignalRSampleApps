...
public sealed class DemoJwtTokenService
{
    private const string Issuer = "signalr-auth-sample";
    private const string Audience = "signalr-auth-sample-client";
    private static readonly byte[] SigningKey = Encoding.UTF8.GetBytes("signalr-auth-sample-demo-signing-key-change-me");
    ...
    public bool TryValidateToken(string token, out DemoJwtUser user)
    {
        ...
        var tenantId = ReadString(payload.RootElement, "tenant_id");
        var role = ReadString(payload.RootElement, "role");

        if (string.IsNullOrWhiteSpace(userId) || string.IsNullOrWhiteSpace(tenantId) || string.IsNullOrWhiteSpace(role))
        {
            return false;
        }

        user = new DemoJwtUser(userId, tenantId, role);
        return true;
    }
...
public readonly record struct DemoJwtUser(string UserId, string TenantId, string Role);