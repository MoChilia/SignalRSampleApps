...
async function connect() {
  ...
  const token = await generateToken(hubUrl, userId, tenantId, role);
  ...
  try {
    setBusy(true);
    log(`[connect] user=${userId}, tenant=${tenantId}, role=${role}`);
...
function setConnected(isConnected) {
  ...
  tenantIdInput.disabled = isConnected;
...
async function generateToken(hubUrl, userId, tenantId, role) {
