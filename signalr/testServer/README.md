...
The reusable Node client lives in `../../shared/signalr-client` and can connect to either this self-hosted server or the Azure SignalR Service-backed server.

Transport-specific clients are split into separate files in the shared client folder:
...
Or run the shared client directly:
